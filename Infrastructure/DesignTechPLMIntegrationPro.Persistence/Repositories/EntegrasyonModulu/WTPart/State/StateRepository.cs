...
namespace DesignTechPLMIntegrationPro.Persistence.Repositories.EntegrasyonModulu.WTPart.State
{
    public class StateRepository : IStateService
    {
        ...
        public StateRepository(IDbConnection dbConnection, ILogService logService, ApiService apiService, IModuleSettingsService moduleSettingsService)
        {
        ...
        private async Task<bool> IsModuleProcessingEnabled()
        {
            return await _moduleSettingsService.GetSettingValueAsync("EnableEntegrationModuleProcessing");
        }
        public async Task CANCELLED()
        {
            if (await IsModuleProcessingEnabled())
            {
                ...
                while (true)
                {
                    ...
                    using (var conn = new SqlConnection("Persist Security Info=False;User ID=oozcelik;Password=Onur.!!35;Initial Catalog=PLM1;Server=192.168.1.11;TrustServerCertificate=True"))
                    {
                        ...
                        DELETE FROM [PLM1].[Des_LogDataTrackingProcess]
                         WHERE [PartID] = @PartID", new { partItem.PartID, });
                                }
                            }
